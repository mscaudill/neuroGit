function standardOsiHist( cellTypesofInterest)
...
% the (rPref-rNull)/(rPref+rNull) method.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% calculate osi for each array in AreasofInterest{cellTypeIndex} using
% standard method (Rpref-RNull)/(Rpref+RNull)
osis{cellTypeIndex} = cellfun(@(x) (max(x)-min(x))/(max(x)+min(x)),...
...
xx = linspace(0,max(max(osisMat)),1000);
