function csTuningWidthsHist(cellTypeofInterest)
% csTuningWidthsHist calculates the center and surround widths of the areas
...
% from a double gaussian fit to the center or cross data.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
centerHWHMs(centerExclusions)=[];

% now repeat for the surround widths
surroundHWHMs = cellfun(@(x) sqrt(2*log(2))*x(3), crossFitParams);
surroundHWHMs(surroundExclusions) = [];

% Bundle the HWHMs into a matrix
allHWHMs = [centerHWHMs',surroundHWHMs'];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% PLOT
figure(1)
[hts,ctrs] = hist(allHWHMs,[0:5:80]);
bar(ctrs,hts,'hist')

centerArea = sum(hts(:,1)) * (ctrs(2)-ctrs(1));
surroundArea = sum(hts(:,2)) * (ctrs(2)-ctrs(1));

xx = linspace(0,80);

hold on; 

plot(xx,centerArea*normpdf(xx,mean(centerHWHMs),std(centerHWHMs)),'b-')
hold on;

plot(xx,surroundArea*normpdf(xx,mean(surroundHWHMs),std(surroundHWHMs)),'r-')
