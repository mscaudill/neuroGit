function centerIsoScatter(cellTypesofInterest)
% centerIsoScatter creates a population scatterPlot of isoOriented areas
% vs center areas.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% mean areas across all the roiSets
...
filteredThetas{cellTypeIndex} = csCellArrayFilter(allMaxThetas,...
                            allCellTypes,...
                            cellTypeofInterest,...
                            allResponsePatterns,respPattern);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%% OBTAIN CENTER AND ISO ORIENTED AREAS %%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
centerAreas{cellTypeIndex} = cellfun(@(g) cellfun(@(t) t(1)/(2*7.81), g),...
                            filteredAreas{cellTypeIndex}, 'UniformOut',0); 

% now obtain the iso-oriented areas
isoAreas{cellTypeIndex} = cellfun(@(h) cellfun(@(s) s(4)/(2*7.81), h),...
...
for roi=1:numel(centerAreas{cellTypeIndex})
    maxCenterAreas{cellTypeIndex}(roi) =...
        centerAreas{cellTypeIndex}{roi}(maxIndices{cellTypeIndex}(roi));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
meanMaxCenterArea = cellfun(@(x) mean(x), maxCenterAreas);
semMaxCenterArea = cellfun(@(x) std(x),...
                        maxCenterAreas)./sqrt(numel(maxCenterAreas));
...
for cellType = 1:numel(cellTypesofInterest)
    ...
    scatter(maxCenterAreas{cellType}, maxIsoAreas{cellType}, 50,...
        colors{cellType})
    
    scatter(meanMaxCenterArea(cellType),meanMaxIsoArea(cellType),...
        150,colors{cellType},'^','fill')
    % call the exchange function errorbarxy to plot mean and sems
    errorbarxy(meanMaxCenterArea(cellType), meanMaxIsoArea(cellType),...
        semMaxCenterArea(cellType), semMaxIsoArea(cellType),[],[],...
...
title(['Suppression Scatter (', cellTypesofInterest{1}, ') n = ',...
        num2str(numel(maxCenterAreas{1})),...
        ' (',cellTypesofInterest{2},') n= ',...
        num2str(numel(maxCenterAreas{2}))]);
